...
from designkp_backend.db.models.catalog import (
    SubtractorPartGroup,
    SubtractorPartGroupItem,
    SubtractorPartGroupParamDefault,
    SubtractorPartGroupParamGroup,
    Param,
    ParamGroup,
    PartFormula,
    PartKind,
...
SUBTRACTOR_GROUP_CONTROLLER_TYPE_WIDTH_NO_TOP = "width_controller_internal_group_part"
SUBTRACTOR_GROUP_CONTROLLER_TYPE_WIDTH_NO_TOP_RIGHT = "width_controller_internal_group_part_right"
...
def _normalize_hex_color(value: str | None, fallback: str = DEFAULT_SUBTRACTOR_LINE_COLOR) -> str:
    ...
    if not normalized or len(normalized) != 7:
        raise HTTPException(
            ...
            detail="Subtractor part group line color must be a HEX value like #8A98A3.",
        )
...
class SubtractorPartGroupPartSelectionPayload(BaseModel):
    part_formula_id: int = Field(ge=1)
    enabled: bool = True
    ui_order: int = Field(default=0, ge=0)


class SubtractorPartGroupPartItem(BaseModel):
    id: uuid.UUID
    part_formula_id: int
    part_kind_id: int
    part_code: str
...
class SubtractorPartGroupParamGroupSelectionPayload(BaseModel):
    param_group_id: int = Field(ge=1)
    enabled: bool = True
    ui_order: int = Field(default=0, ge=0)


class SubtractorPartGroupParamGroupItem(BaseModel):
    id: uuid.UUID
    param_group_id: int
...
class SubtractorPartGroupItemResponse(BaseModel):
    id: uuid.UUID
    ...
    sort_order: int
    is_system: bool
    parts: list[SubtractorPartGroupPartItem]
    param_groups: list[SubtractorPartGroupParamGroupItem]
    param_defaults: dict[str, str | None]
    param_overrides: dict[str, "SubtractorPartGroupParamOverrideItem"]
    controller_type: str | None = None
    controller_bindings: dict[str, "SubtractorPartGroupControllerBindingItem"]

    model_config = {"from_attributes": True}


class SubtractorPartGroupParamOverrideItem(BaseModel):
...
class SubtractorPartGroupControllerBindingItem(BaseModel):
    param_code: str | None = None


class SubtractorPartGroupCreate(BaseModel):
    ...
    line_color: str = Field(default=DEFAULT_SUBTRACTOR_LINE_COLOR, min_length=7, max_length=7)
    sort_order: int | None = Field(default=None, ge=0)
    is_system: bool = False
    parts: list[SubtractorPartGroupPartSelectionPayload] = Field(default_factory=list)
    param_groups: list[SubtractorPartGroupParamGroupSelectionPayload] = Field(default_factory=list)
    param_defaults: dict[str, str | int | float | bool | None] = Field(default_factory=dict)
    param_overrides: dict[str, "SubtractorPartGroupParamOverridePayload"] = Field(default_factory=dict)
    controller_type: str | None = Field(default=None, max_length=128)
    controller_bindings: dict[str, "SubtractorPartGroupControllerBindingPayload"] = Field(default_factory=dict)


class SubtractorPartGroupUpdate(BaseModel):
    admin_id: uuid.UUID | None = None
...
class SubtractorPartGroupParamOverridePayload(BaseModel):
...
class SubtractorPartGroupControllerBindingPayload(BaseModel):
    param_code: str | None = Field(default=None, max_length=128)


SubtractorPartGroupItemResponse.model_rebuild()


async def _next_group_id(session: AsyncSession) -> int:
    max_id = await session.scalar(select(func.max(SubtractorPartGroup.group_id)))
    return int(max_id or 0) + 1


async def subtractor_part_group_param_groups_table_ready(session: AsyncSession) -> bool:
    result = await session.execute(select(func.to_regclass("subtractor_part_group_param_groups")))
    return bool(result.one()[0])


async def subtractor_part_group_param_defaults_table_ready(session: AsyncSession) -> bool:
    result = await session.execute(select(func.to_regclass("subtractor_part_group_param_defaults")))
    return bool(result.one()[0])
...
def _normalize_controller_bindings_payload(
    ...
    bindings: dict[str, SubtractorPartGroupControllerBindingPayload] | dict[str, object] | None,
    ...
    for slot in allowed_slots:
        ...
        if isinstance(raw_value, SubtractorPartGroupControllerBindingPayload):
...
async def _ensure_unique_group_code(session: AsyncSession, *, code: str, exclude_id: uuid.UUID | None = None) -> None:
    stmt = select(SubtractorPartGroup).where(SubtractorPartGroup.code == code)
    if exclude_id is not None:
        stmt = stmt.where(SubtractorPartGroup.id != exclude_id)
    duplicate = await session.scalar(stmt)
    if duplicate:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Subtractor part group code is already used.")
...
async def _load_group(
    ...
    *,
    ...
    if include_param_groups is None:
        include_param_groups = await subtractor_part_group_param_groups_table_ready(session)
    if include_param_defaults is None:
        include_param_defaults = await subtractor_part_group_param_defaults_table_ready(session)
    stmt = select(SubtractorPartGroup).options(selectinload(SubtractorPartGroup.parts))
    if include_param_groups:
        stmt = stmt.options(selectinload(SubtractorPartGroup.param_groups))
    if include_param_defaults:
        stmt = stmt.options(selectinload(SubtractorPartGroup.param_defaults).selectinload(SubtractorPartGroupParamDefault.param))
    item = await session.scalar(stmt.where(SubtractorPartGroup.id == group_uuid))
    if not item:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Subtractor part group not found.")
    return item


def _serialize_group(
    item: SubtractorPartGroup,
    *,
    include_param_groups: bool = True,
    include_param_defaults: bool = True,
) -> SubtractorPartGroupItemResponse:
    ...
    return SubtractorPartGroupItemResponse(
        id=item.id,
        ...
        line_color=_normalize_hex_color(getattr(item, "line_color", DEFAULT_SUBTRACTOR_LINE_COLOR), DEFAULT_SUBTRACTOR_LINE_COLOR),
        sort_order=item.sort_order,
        is_system=item.is_system,
        parts=[
            SubtractorPartGroupPartItem.model_validate(part)
            for part in sorted(item.parts, key=lambda part: (int(part.ui_order), int(part.part_formula_id)))
        ],
        param_groups=[
            SubtractorPartGroupParamGroupItem.model_validate(param_group)
...
async def _require_accessible_subtractor_part_formulas(
    session: AsyncSession,
    *,
    admin_id: uuid.UUID | None,
...
async def _replace_group_parts(
    ...
    *,
    ...
    parts: list[SubtractorPartGroupPartSelectionPayload],
) -> None:
    next_formulas = await _require_accessible_subtractor_part_formulas(
        session,
    ...
    for selection in parts:
        ...
        if not formula:
            ...
            continue
        if existing is None:
            existing = SubtractorPartGroupItem(
                group=group,
...
async def _replace_group_param_groups(
    ...
    *,
    group: SubtractorPartGroup,
    param_groups: list[SubtractorPartGroupParamGroupSelectionPayload],
) -> None:
    ...
    for selection in param_groups:
        ...
        if existing is None:
            existing = SubtractorPartGroupParamGroup(
                group=group,
...
async def _params_for_subtractor_group(session: AsyncSession, group: SubtractorPartGroup) -> list[Param]:
    await session.flush()
    selected_param_group_ids = sorted({
        int(row.param_group_id)
        for row in (
            await session.scalars(
                select(SubtractorPartGroupParamGroup).where(
                    SubtractorPartGroupParamGroup.group_ref_id == group.id
                )
            )
        ).all()
...
async def _sync_group_param_defaults(session: AsyncSession, group: SubtractorPartGroup) -> bool:
    changed = False
    params = await _params_for_subtractor_group(session, group)
    ...
    defaults = (
        await session.scalars(
            select(SubtractorPartGroupParamDefault).where(SubtractorPartGroupParamDefault.subtractor_part_group_id == group.id)
        )
    ).all()
...
async def _apply_group_param_defaults(
    ...
    param_overrides: dict[str, SubtractorPartGroupParamOverridePayload],
) -> None:
    ...
    for param_code, raw_value in param_defaults.items():
        ...
        if not param:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Unknown param code for this subtractor group scope: {param_code}")
...
async def _apply_group_controller_config(
    ...
    *,
    controller_type: str | None,
    controller_bindings: dict[str, SubtractorPartGroupControllerBindingPayload],
) -> None:
    normalized_type = _normalize_controller_type(controller_type)
    ...
    for slot, binding in normalized_bindings.items():
        ...
        if param_code and param_code not in allowed_codes:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Unknown controller param code for this subtractor group scope: {param_code}",
            )
...
@router.get("", response_model=list[SubtractorPartGroupItemResponse])
async def list_subtractor_part_groups(
    admin_id: uuid.UUID | None = Query(default=None),
    session: AsyncSession = Depends(get_db_session),
) -> list[SubtractorPartGroupItemResponse]:
    await require_admin_if_present(session, admin_id)
    include_param_groups = await subtractor_part_group_param_groups_table_ready(session)
    include_param_defaults = include_param_groups and await subtractor_part_group_param_defaults_table_ready(session)
    stmt = select(SubtractorPartGroup).options(selectinload(SubtractorPartGroup.parts))
    if include_param_groups:
        stmt = stmt.options(selectinload(SubtractorPartGroup.param_groups))
    if include_param_defaults:
        stmt = stmt.options(selectinload(SubtractorPartGroup.param_defaults).selectinload(SubtractorPartGroupParamDefault.param))
    if admin_id is None:
        stmt = stmt.where(SubtractorPartGroup.admin_id.is_(None))
    else:
        stmt = stmt.where(or_(SubtractorPartGroup.admin_id.is_(None), SubtractorPartGroup.admin_id == admin_id))
    stmt = stmt.order_by(
        SubtractorPartGroup.is_system.desc(),
        SubtractorPartGroup.sort_order.asc(),
        SubtractorPartGroup.group_id.asc(),
    )
...
@router.post("", response_model=SubtractorPartGroupItemResponse, status_code=status.HTTP_201_CREATED)
async def create_subtractor_part_group(payload: SubtractorPartGroupCreate, session: AsyncSession = Depends(get_db_session)) -> SubtractorPartGroupItemResponse:
    ...
    if not code:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Subtractor part group code is required.")
    ...
    item = SubtractorPartGroup(
        ...
        line_color=_normalize_hex_color(payload.line_color, DEFAULT_SUBTRACTOR_LINE_COLOR),
...
@router.patch("/{group_uuid}", response_model=SubtractorPartGroupItemResponse)
async def update_subtractor_part_group(
    group_uuid: uuid.UUID,
    ...
    item.line_color = _normalize_hex_color(payload.line_color, DEFAULT_SUBTRACTOR_LINE_COLOR)
...
async def delete_subtractor_part_group(group_uuid: uuid.UUID, session: AsyncSession = Depends(get_db_session)) -> Response:
    item = await session.get(SubtractorPartGroup, group_uuid)
    if not item:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Subtractor part group not found.")
